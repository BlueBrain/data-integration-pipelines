...
def find_mismatches(
    forge: KnowledgeGraphForge, type_: str,
...
def do(forge: KnowledgeGraphForge, types: List, chunk_size=200) -> pd.DataFrame:
    issues = []
    for type_ in types:
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--chunk_size", help="How many resources should be searched at once",
    ...
    group.add_argument(
        "--typelist", help="The list of types to check",
        type=str
    )
    group.add_argument(
        "--typelist_filepath", help="A json file containing the list of types to check",
        type=str
    )
    ...
    df = do(forge_bucket, types, received_args.chunk_size)
