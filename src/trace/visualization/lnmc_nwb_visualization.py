...
def nwb2png(nwb_path, nwb, png_dir):
    ...
    for stimulus in stimuli:
        ...
        for repetition in repetitions:
            trials_response = trials_df[(trials_df[('sequential_recordings','stimulus_type')] == stimulus)][(trials_df[('repetitions','id')] == repetition)][('responses','response')]
            trials_stimulus = trials_df[(trials_df[('sequential_recordings','stimulus_type')] == stimulus)][(trials_df[('repetitions','id')] == repetition)][('stimuli','stimulus')]
            ...
            for item in trials_response.iteritems():
                if item[1][2]:
                    ...
                    time_unit = item[1][2].starting_time_unit
        ...
        for repetition in set(subset_df[('repetitions','id')]):
            for i in range(len(subset_df[(subset_df[('repetitions','id')] == repetition)][('responses','response')])):
                if subset_df[(subset_df[('repetitions','id')] == repetition)][('stimuli','stimulus')].iloc[i][2] is not None:
                    current = list(map(float, (subset_df[(subset_df[('repetitions','id')] == repetition)][('stimuli','stimulus')].iloc[i][2].data * subset_df[(subset_df[('repetitions','id')] == repetition)][('stimuli','stimulus')].iloc[i][2].conversion)))
                else:
                    current = None
                if subset_df[(subset_df[('repetitions','id')] == repetition)][('responses','response')].iloc[i][2] is not None:
                    voltage = list(map(float, (subset_df[(subset_df[('repetitions','id')] == repetition)][('responses','response')].iloc[i][2].data * subset_df[(subset_df[('repetitions','id')] == repetition)][('responses','response')].iloc[i][2].conversion)))
                else:
                    voltage = None
